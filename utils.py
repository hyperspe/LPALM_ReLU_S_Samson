...
def split_A(dataset):
	...
	for i,(X,A,S) in enumerate(dataset):
		crit = np.sum(np.multiply(A0[:,0],A[:,0])) / (np.linalg.norm(A0[:,0]) * np.linalg.norm(A[:,0])) + np.sum(np.multiply(A0[:,1],A[:,1])) / (np.linalg.norm(A0[:,1]) * np.linalg.norm(A[:,1])) + np.sum(np.multiply(A0[:,2],A[:,2])) / (np.linalg.norm(A0[:,2]) * np.linalg.norm(A[:,2])) #Modifiée ici pour 3 au lieu de 4
		l.append(crit)
	ind = sorted(range(len(l)), key=lambda k: l[k])
	val_idx = ind[:int(len(l)*0.2)]
	train_idx = ind[int(len(l)*0.2):]
	datasets = {}
	datasets['train'] = Subset(dataset, train_idx)
	datasets['val'] = Subset(dataset, val_idx)
